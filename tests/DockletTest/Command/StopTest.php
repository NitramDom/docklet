...
class StopTest extends DockerTestCase
{
    ...
    public function testStopById()
    {
        ...
        $containers = static::dockerPs($runningOnly = true);
    ...
    public function testStopByName()
    {
        $containerName = 'testTest';
        $options = Run::options(static::$image, static::$container_command)
            ->name($containerName)
            ->daemon(true);

        static::$docker->run($options);

        $containers = static::dockerPs($runningOnly = true);
        $found = preg_match('/'. $containerName .'/m', $containers);
        $this->assertTrue(filter_var($found, FILTER_VALIDATE_BOOLEAN));

        static::$docker->stop($containerName);

        $containers = static::dockerPs($runningOnly = true);
        $found = preg_match('/'. $containerName .'/m', $containers);
        $this->assertFalse(filter_var($found, FILTER_VALIDATE_BOOLEAN));

        static::$removeContainersWithId[] = $containerName;
    }
}
